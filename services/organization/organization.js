...
// MODELS IMPORT
const Category = require('../../models/db/organization/organization.model');
    ...
    try {
        ...
        try {
            var result = await categoryModel.save()
                .then(doc => {
                    return { isSuccess: true, message: "Organization Created Successfully!!", data: doc };
                }).catch(err => {
                    return { isSuccess: false, message: "Failed to Created Organization!!", data: err };
                });
            return result;
        } catch (err) {
            return { isSuccess: false, data: { code: err.code, errmsg: err.code == 11000 ? "Organization Name Already Exists" : err.errmsg }, message: '' };
        }
    }
    catch (ex) {
...
async function Update(req) {
    try {
        ...
        var result = await Category.findByIdAndUpdate(categoryId, data, { new: true })
            .then(res => {
                return { isSuccess: true, message: "Organization Updated successfully!!", data: res };
            })
            .catch(err => {
                return { isSuccess: false, message: "Failed to Update Organization!!", data: err };
            });
        return result;
    }
    catch (ex) {
        return { isSuccess: false, message: "Failed to Update Organization!!", data: ex };
    }
}
...
async function Delete(req) {
    try {
        ...
        var result = await Category.findByIdAndUpdate({ _id: categoryId }, { isActive: false, isDeleted: true, updatedDate: moment.utc() }, { new: true })
            .then(res => {
                return { isSuccess: true, message: "Organization deleted successfully!!", data: res };
            })
            .catch(err => {
                return { isSuccess: false, message: "Failed to delete Organization!!", data: err }
            });
        return result;
    }
    catch (ex) {
        return { isSuccess: false, message: "Failed to delete Organization!!", data: ex };
    }
}
...
async function GetAll(req) {
    try {
        ...
        return {
            isSuccess: false,
            message: 'Exception while getting Organization!!',
            data: ex,
            currentPage: pageNumber,
            pageSize: pageSize,
            totalRecords: 0
...
async function GetById(id) {
    try {
        var result = await Category.findOne({
            ...
            .then(res => {
                if (res !== null) {
                    return { isSuccess: true, message: 'Organization Retrived Successfully!!', data: res };
                }
                else {
                    return { isSuccess: false, message: 'Invalid Organization ID!!', data: res };
                }
            })
            .catch(err => {
                return { isSuccess: false, message: 'Not found Organization details!!', data: err };
            });
        return result;
    } catch (ex) {
        return { isSuccess: false, message: 'Error to get Organization!!', data: ex };
    }
}
