...
// MODELS IMPORT
const Category = require('../../models/db/category/category.model');
    ...
    try {
        ...
        try {
            var result = await categoryModel.save()
                .then(doc => {
                    return { isSuccess: true, message: "Category Created Successfully!!", data: doc };
                }).catch(err => {
                    return { isSuccess: false, message: "Failed to Created Category!!", data: err };
                });
            return result;
        } catch (err) {
            return { isSuccess: false, data: { code: err.code, errmsg: err.code == 11000 ? "Category Name Already Exists" : err.errmsg }, message: '' };
        }
    }
    catch (ex) {
...
async function Update(req) {
    try {
        ...
        var result = await Category.findByIdAndUpdate(categoryId, data, { new: true })
            .then(res => {
                return { isSuccess: true, message: "Category Updated successfully!!", data: res };
            })
            .catch(err => {
                return { isSuccess: false, message: "Failed to Update Category!!", data: err };
            });
        return result;
    }
    catch (ex) {
        return { isSuccess: false, message: "Failed to Update Category!!", data: ex };
    }
}
...
async function GetAll(req) {
    try {
        ...
        return {
            isSuccess: false,
            message: 'Exception while getting Category!!',
            data: ex,
            currentPage: pageNumber,
            pageSize: pageSize,
            totalRecords: 0
...
async function GetById(id) {
    try {
        var result = await Category.findById(id)
            ...
            .then(res => {
                if (res !== null) {
                    return { isSuccess: true, message: 'Category Retrived Successfully!!', data: res };
                }
                else {
                    return { isSuccess: false, message: 'Invalid Category ID!!', data: res };
                }
            })
            .catch(err => {
                return { isSuccess: false, message: 'Not found Category details!!', data: err };
            });
        return result;
    } catch (ex) {
